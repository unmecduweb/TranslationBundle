...
class XliffDumper implements DumperInterface {
    ...
    public function dump(MessageCatalogue $catalogue, $domain = 'messages') {
        ...
        $root->setAttribute('xmlns:LI', 'urn:LI:translation');
                ...
                if ($sources = $message->getSources()) {
                    foreach ($sources as $source) {
                        if ($source instanceof FileSource) {
                            $unit->appendChild($refFile = $doc->createElement('LI:reference-file', $source->getPath()));
                        ...
                        $unit->appendChild($doc->createElementNS('LI:reference', (string) $source));
                    }
                }
